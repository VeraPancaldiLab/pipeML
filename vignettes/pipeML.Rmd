...
```{r, eval = FALSE}
pred = pipeML::compute_prediction(res, testing_set, traitData$Best.Confirmed.Overall.Response, "CR", file.name = "Test", maximize = "Accuracy", return = F)
```
...
```{r, eval = FALSE}
get_curves(pred$Metrics, "Specificity", "Sensitivity", "Recall", "Precision", "Cohort", pred$AUC$AUROC, pred$AUC$AUPRC, "Test")
```
...
for (cohort in unique(traitData$Cohort)) {
  ...
  #### ML Training
  res = pipeML::compute_features.training.ML(features_train, traitData_train$Response, "R", metric = "AUROC", 
                                             stack = T, k_folds = 2, n_rep = 3, feature.selection = F, 
                                             seed = 123, LODO = TRUE, batch_id = traitData_train$Cohort, 
                                             ncores = 3, return = F)
  ...
  #### Testing
  pred = pipeML::compute_prediction(res, features_test, traitData_test$Response, 
                                    trait.positive = "R", stack = TRUE, return = F)
...
Plot results: For plotting we will make use of our `get_curves` function adapted in a case for multiple cohorts.
```{r}
metrics = list()
auroc = c() 
auprc = c()
...
get_curves(metrics, "Specificity", "Sensitivity", "Recall", "Precision", "Cohort", metrics$AUROC, metrics$AUPRC, "Test")
```
